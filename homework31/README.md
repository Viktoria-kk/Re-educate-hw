# Homework 31 — Swagger API documentation

Copied from homework30. All 15 movie, director and photo routes are documented with request examples, filters, UUID parameters, response schemas and error statuses.

## Swagger

Run `npm install`, configure `.env` as described below, then run `npm run start:dev`.

- Swagger UI: http://localhost:3000/api
- OpenAPI JSON: http://localhost:3000/api-json
- Use **Try it out** to call routes, including multipart photo uploads. Create a director first and use its UUID when creating a movie.
- Swagger 11 is used to match NestJS 11: `npm install --save @nestjs/swagger@11`.
- PATCH schemas inherit Swagger metadata using `PartialType` from `@nestjs/swagger`; all fields are optional and null values remain invalid.

## Homework 30 — Director profile photos and movie galleries

Adapted from homework29 with AWS S3 image uploads, CloudFront delivery, photo deletion and retryable cleanup. Directors have one replaceable profile photo; movies accept single and multiple photos, up to 10 total.

**Read [PHOTO_GUIDE.md](PHOTO_GUIDE.md) for the complete setup, endpoints, Postman/curl examples, exact AWS policies, tests and implementation details.**

The existing movie/director JSON API is preserved below. For homework30, use the AWS variables in `.env.example` as well as your existing MySQL settings. The copied database setting is unchanged; `SQL_SYNC=true` adds the photo columns and cleanup table when the application starts.

Checks: `npm run build`, `npm run lint`, `npm test -- --runInBand`, `npm run test:e2e -- --runInBand`, `npm run test:cov`, `npm run test:mysql`, and the live AWS check `npm run test:aws`.
