# Homework 29 — Jest service tests

Copied from homework28, with unit tests for every application service: `MoviesService` and `DirectorsService`.

## Jest tests based on lec20

The lecture uses nested `describe`/`it` blocks, `beforeEach`, Nest's `Test.createTestingModule`, mocked providers supplied through `useValue`, `jest.fn()` and `jest.spyOn()`. Tests check returned values, dependency calls, and exceptions. Its utility tests demonstrate `toBe`, `toEqual`, `toBeCloseTo`, and `toThrow`; its HTTP test uses Supertest.

Here, TypeORM's `getRepositoryToken` replaces the lecture's Mongoose `getModelToken`. Each service has a neighboring `.service.spec.ts` file. Mocks and fixtures are reset before each test, and asynchronous rejection assertions are awaited. Query builder mocks verify bound filter values, joins, ordering and pagination. The existing integration tests exercise the actual SQL queries and HTTP validation.

- `npm test -- --runInBand`: run both service unit suites without MySQL or `.env`.
- `npm run test:cov -- --runInBand`: report coverage for all service files (100% statements, lines and functions; at least 90% branches, including generated decorator metadata).
- `npm run test:watch`: watch unit tests.
- `npm run test:e2e -- --runInBand`: run the copied SQL.js API integration suite.
- `npm run test:mysql`: run the integration suite against a temporary MySQL database.

Both service suites cover create, findAll, findOne, update and remove, including missing records, database failures, relationships, combined filters, single/equal/reversed year boundaries, empty results, pagination and partial updates. Movies also test missing directors and reassignment.

The copied `.env` keeps the existing local homework28 connection; service unit tests never connect to it. The original homework28 folder is unchanged.

## Original API and setup
